...
Matrix* get_result(Matrix* mat,int* swap_map){
    ...
    double sum =0;
    // 计算xn的值
    m_set(res,swap_map[mat->m-1], 0, m_get(mat,mat->m-1,mat->m) / m_get(mat,mat->m-1,mat->m-1));
    // 计算每一行
    for(int xind = mat->m-2;xind > -1;xind--){
        double sum =0;
        // 计算和的话,从<xind,xind+1>往后
        for(int ind = xind+1;ind < mat->m;ind++){
            sum += m_get(mat,xind,ind) * m_get(res,swap_map[ind],0);
        }
...
Matrix* tran(Matrix* mat){
    Matrix* res = m_create(mat->n,mat->m);
    for(int i = 0; i < mat->m;i++){
        for(int j = 0;j < mat->n;j++){
            m_set(res,i,j,m_get(mat,j,i));
        }
    }
    return res;
}